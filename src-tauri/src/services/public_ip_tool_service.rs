...
impl PublicIpToolService {
    ...
    pub async fn fetch_public_ip_info() -> AppResult<PublicIpInfo> {
        ...
        let ipv6 = Self::fetch_ip_from_endpoint(&client, "https://api6.ipify.org?format=json").await;

        let geo_target_ip = current_ip
            .clone()
            .or_else(|| ipv4.clone())
            .or_else(|| ipv6.clone())
            .ok_or_else(|| AppError::NetworkRequestFailed("未能获取任何公网 IP 地址".to_string()))?;

        let geo_response = client
            .get(format!("https://ipwho.is/{}", geo_target_ip))
            .send()
            .await
            .map_err(|error| AppError::NetworkRequestFailed(format!("获取归属信息失败: {}", error)))?;

        let geo_body = geo_response
            .text()
            .await
            .map_err(|error| AppError::NetworkRequestFailed(format!("读取归属信息响应失败: {}", error)))?;
